...
class Wonkasoft_GF_Order_Select extends GF_Field {

	public $type = 'order_select';

	public function get_form_editor_field_title() {
		return esc_attr__( 'Order Select', 'gravityforms' );
	}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		foreach ( $customer_orders as $order_number ) {
			...
			array_push(
				$this->choices,
				array(
					'text'  => $seq_order_number,
					'value' => $seq_order_number,
				)
			);
		}
		...
		$css_class          = trim( esc_attr( $class ) . ' gfield_order_select' );
...
GF_Fields::register( new Wonkasoft_GF_Order_Select() );
