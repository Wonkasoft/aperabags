...
<div class="top-page-slide">
				...
				if ( isset( $_SERVER['HTTP_USER_AGENT'] ) ) :
					$user_agent = esc_html_raw( $_SERVER['HTTP_USER_AGENT'] );
					$edge_class = ( preg_match( '/Edge/', $user_agent ) ) ? ' is-edge' : '';
					$ios_class  = ( preg_match( '/iPad|iPod|iPhone/', $user_agent ) ) ? ' is-ios' : '';
				endif;
				$browser_classes = $edge_class . '' . $ios_class;
				if ( wp_is_mobile() && ! preg_match( '/iPad/', $user_agent ) ) :
					?>
<div class="top-slide-img-holder<?php esc_attr_e( $ios_class ); ?>" data-img-url="<?php esc_attr_e( $slide->slide_mobile_img ); ?>" style="background-image:url('<?php echo $slide->slide_mobile_img; ?>');">
					...
					?>
<div class="top-slide-img-holder<?php echo esc_attr__( "$ios_class" ); ?>" data-video-url="<?php echo esc_attr__( "$slide->slide_img" ); ?>">
